...
=======
import joblib
>>>>>>> 0948b35 (Add AI tooth measurement models)
...
def extract_features_for_prediction(img, x, y, w_px, h_px):
    """Extract features from a detected region for AI prediction"""
    x, y = max(0, x), max(0, y)
    ...
    features.append(0.75)  # default solidity
...
<<<<<<< HEAD
    try:
        # 1. Validate
=======
    """Original endpoint — kept exactly as before"""
    try:
>>>>>>> 0948b35 (Add AI tooth measurement models)
...
=======
        ...
        if not candidates:
            return jsonify({
                ...
                'findings':'No significant bone defect region detected.',
                'debug':{'contoursTotal':total,'contoursKept':0,
                         'imageSize':f'{img_w_px} x {img_h_px} px',
...
=======
        scaffold_range, material = recommend_scaffold(volume)
        severity = severity_label(volume)
        findings = (f'{severity} bone defect detected. '
                    f'Dimensions: {width} mm (W) × {height} mm (H) × {depth} mm (D). '
                    f'Volume: {volume} mm³.')
        ...
        return jsonify({
            ...
            'debug':{
                ...
                'best_area_px2':round(best_area,0),
                'raw_w_mm':round(raw_w,2),'raw_h_mm':round(raw_h,2),'raw_d_mm':round(raw_d,2),
...
def ai_analyze():
    """NEW endpoint — AI-powered tooth measurements"""
    try:
        if not ai_ready:
        ...
        for c in contours:
            ...
            # Extract features and run AI prediction
